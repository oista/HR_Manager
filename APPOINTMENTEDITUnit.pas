...
begin
  ...
  //  ShowMessage(ADQuery1WORKCONTRACTID.asstring);
   end;
  ADQuery1.Close;

  // узнаем новый перевод или продолжение старого
  OldEmp:= PKDBTable2.Fields.FieldByName('ID').FieldVal;
    ...
    tcInsert:
     begin
      if (ProcState=NEW) then
        begin
         if CheckBox3.Checked then    // создаем сразу в активном статусе или в проетном
...
procedure TAPPOINTMENTEDIT.CheckBox3Click(Sender: TObject);
begin
  if CheckBox3.Checked then
