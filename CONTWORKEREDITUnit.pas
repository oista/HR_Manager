...
type
  ...
  TCONTWORKEREDIT = class(TEditDialog)
    ...
    procedure PKDBDictEdit2BeforeOpenButtonClick(Sender: TObject;
      Form: TVACANCY);
...
procedure TCONTWORKEREDIT.PKDBDictEdit2BeforeOpenButtonClick(Sender: TObject;
  Form: TVACANCY);
begin       // меняем вакансии на список, включающий внештатный состав
  form.MainQuery.Close;
  form.MainQuery.SQL.Text:='select * from HR_V_VACANCY_CONTWORKER';
  form.MainQuery.Open;
  // открываем возможность редактировать вакансии (только для внештатного состава)
  form.PKDBEditButtons1.EditDialogClassName:='TVACANCYEDIT';
end;

procedure TCONTWORKEREDIT.RunCommand;
begin
   // копируем работодателя в приказ
  PKDBDictEdit10.DictionaryValue := PKDBBLabelCombobox4.selectid;

  // даты начала-окончания работ и договора идентичны
  if (PKDBLabelEdit10.Text='') and (PKDBLabelEdit6.Text<>'') then
     PKDBLabelEdit10.Text := PKDBLabelEdit6.Text
  else
     if (PKDBLabelEdit6.Text='') and (PKDBLabelEdit10.Text<>'') then
     PKDBLabelEdit6.Text := PKDBLabelEdit10.Text;

  if (PKDBLabelEdit14.Text='') and (PKDBLabelEdit7.Text<>'') then
     PKDBLabelEdit14.Text := PKDBLabelEdit7.Text
  else
     if (PKDBLabelEdit7.Text='') and (PKDBLabelEdit14.Text<>'') then
     PKDBLabelEdit7.Text := PKDBLabelEdit14.Text;
     //-----------------------------------------------
